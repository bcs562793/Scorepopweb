/* ═══════════════════════════════════════════════════════
   SCOREPOP — bball.js  (v3.0 — liste sayfası)
   Modal kaldırıldı; maç tıklaması /basketbol/mac/[id]-slug adresine yönlendirir.
═══════════════════════════════════════════════════════ */
'use strict';
...
function fmtTime(isoStr) {
  if (!isoStr) return '--:--';
  try { const d = new Date(isoStr); return `${String(d.getHours()).padStart(2,'0')}:${String(d.getMinutes()).padStart(2,'0')}`; } catch { return '--:--'; }
}
...
function makeSlug(...parts) {
  return parts.filter(Boolean).join('-vs-')
    .toLowerCase()
...
function bballStatus(m) {
  const LIVE_MAP = {
    '1Q':'Ç1','Q1':'Ç1','2Q':'Ç2','Q2':'Ç2','HT':'DEVRE','HALF':'DEVRE',
    '3Q':'Ç3','Q3':'Ç3','4Q':'Ç4','Q4':'Ç4','OT':'UZT','OT1':'UZT1','OT2':'UZT2','LIVE':'CANLI',
  };
...
function buildBballDateStrip() {
  ...
  el.innerHTML = days.map(s =>
    `<button class="bdp${s===B.date?' active':''}" onclick="pickBballDate('${s}')">${s===today?'Bugün':dateLabel(s)}</button>`
  ).join('');
...
async function fetchAllBballRows(query) {
  ...
  while(true) {
    ...
    if (error) { console.error('[bball]',error.message); break; }
...
async function loadBballToday() {
  try {
    const rows = await fetchAllBballRows(
      B.sb.from('live_bball').select('id,nesine_bid,home_team,away_team,league_name,country,status_short,home_score,away_score,home_q1,away_q1,home_q2,away_q2,home_q3,away_q3,home_q4,away_q4,home_ot,away_ot,period,match_clock,scheduled_at,home_avatar,away_avatar,home_recent_form,away_recent_form')
...
async function loadBballArchive(date) {
  ...
  try {
    const res = await fetch(`${BBALL_ARCHIVE_BASE}/${date}/events.json`);
...
async function loadBballFuture(date) {
  ...
  try {
    const rows = await fetchAllBballRows(
      B.sb.from('live_bball').select('id,home_team,away_team,league_name,country,status_short,home_score,away_score,scheduled_at,home_avatar,away_avatar')
...
function renderBballGroup(g) {
  ...
  return `<div class="bball-grp">
    <div class="bball-hdr" onclick="this.closest('.bball-grp').classList.toggle('closed')">
      ...
      ${liveBadge}<span class="bball-arrow">▾</span>
    </div>
...
function renderBballRow(m) {
  ...
  let hcls='', acls='';
  if (st.done && hs!=='—' && as!=='—') {
    if (+hs>+as){hcls='bball-win';acls='bball-loss';} else if (+as>+hs){acls='bball-win';hcls='bball-loss';}
  }

  let qtrsHtml='';
  if (!isNS) {
    const quarters=[[m.home_q1,m.away_q1,'Ç1'],[m.home_q2,m.away_q2,'Ç2'],[m.home_q3,m.away_q3,'Ç3'],[m.home_q4,m.away_q4,'Ç4']];
    if (m.home_ot!=null||m.away_ot!=null) quarters.push([m.home_ot,m.away_ot,'UZT']);
    const qItems = quarters.filter(([h,a])=>h!=null||a!=null).map(([h,a,lbl])=>{
      const isActive=st.live&&((lbl==='Ç1'&&['1Q','Q1'].includes(m.status_short))||(lbl==='Ç2'&&['2Q','Q2'].includes(m.status_short))||(lbl==='Ç3'&&['3Q','Q3'].includes(m.status_short))||(lbl==='Ç4'&&['4Q','Q4'].includes(m.status_short))||(lbl==='UZT'&&m.status_short?.toUpperCase().startsWith('OT')));
      return `<span class="bball-qtr${isActive?' bball-qtr-live':''}"><span class="bball-qtr-lbl">${lbl}</span><span class="bball-qtr-h">${h??'-'}</span><span class="bball-qtr-sep">:</span><span class="bball-qtr-a">${a??'-'}</span></span>`;
    });
  ...
  return `
    <a class="bball-mr${st.live?' is-live':''}" href="${url}" data-id="${m.id}">
      ...
      <div class="bball-team bball-home">
        ...
        <div class="bball-logo-wrap">${hl}</div>
      </div>
      ...
      <div class="bball-team bball-away">
        <div class="bball-logo-wrap">${al}</div>
...
function updateLiveCount(rows) {
  ...
  const bn=document.getElementById('bball-tb-live-n');
...
function startBballCountdown() {
  ...
  B.timer = setInterval(async()=>{
    B.cd--; updateBballRing();
    if (B.cd<=0) { B.cd=B.cycle; await loadBball(true); }
  }, 1000);
}
...
function startBballTick(){if(B.tickTimer)clearInterval(B.tickTimer);}
