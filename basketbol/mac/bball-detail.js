...
function setSEO(row,st){
  ...
  const schema={'@context':'https://schema.org','@type':'SportsEvent',name:`${row.home_team} - ${row.away_team}`,sport:'Basketball',description:desc,url:window.location.href,startDate:startISO,eventStatus:st.live?'https://schema.org/EventLive':(st.done?'https://schema.org/EventCompleted':'https://schema.org/EventScheduled'),homeTeam:{'@type':'SportsTeam',name:row.home_team},awayTeam:{'@type':'SportsTeam',name:row.away_team},...(hasScore?{homeScore:{'@type':'Integer',value:hs},awayScore:{'@type':'Integer',value:as}}:{})};
